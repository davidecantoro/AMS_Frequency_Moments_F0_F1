...
int main(int argc, char *argv[]) {
    ...
    // file di output
    char filename_output[MAXLENGTH] = "naive_f1.csv";
    ...
    while ((opt = getopt(argc, argv, "f:p:s:o:d:qh")) != -1) {
            switch (opt) {
                ...
                case 'q':   // quiet, se attivato non stampa l'output
                    ...
                    break;
                case 'h':
                    printf("Utilizzo: ./naive_f1 [-f nome_file] [-p path] [-o output_file] [-d path_output_file] [-s separatore] [-q] [-h]\n"
    ...
    */
    ...
    if (!quiet) {
        printf("Naive Frequency Moments - momento di ordine 1 \n");    
        printf("Lunghezza stream reale: %d\n", m_len);
