...
mkdir -p "$simulazioni_path/output_algoritmi2"
...
do
  ...
  for algoritmo in ${algoritmi[@]}; do
    ...
    for ((n=1; n<=numero_esecuzioni; n++)); do
      ...
      output=$(gtime -v ./$algoritmo -f $file_name_input -p $simulazioni_path/ -o ${algoritmo}_$file_name_input -d $output_path -q 2>&1)
